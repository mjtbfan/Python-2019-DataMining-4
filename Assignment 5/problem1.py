...
class TicTacToe(BoardGame):
    ...
    def check_game(self,s):
        ...
        e = 2
        board = s.b
        rboard = np.fliplr(s.b)

        # check the 8 lines in the board to see if the game has ended.

        if e == 2:
            diag = board.diagonal()
            rdiag = rboard.diagonal()
            if np.all(diag == 1) or np.all(rdiag == 1):
                e = 1
            if np.all(diag == -1) or np.all(rdiag == -1):
                e = -1
...
class MMNode(Node):
    ...
    def expand(self,g):
       ...
       |-------------------------------
        ...
        children_data = g.get_move_state_pairs(self.s)
        ...
        for i in children_data:
            ...
            children.append(MMNode(i[1], self, [], i[0]))
    ...
    def compute_v(self,g):
        ...
        #       (2.2.2) if it is O player's turn, the value of the current node is the min of all children node's values 
            if self.s.x == 1:
                self.v = max(i.v for i in self.c)
            else:
                self.v = min(i.v for i in self.c)
