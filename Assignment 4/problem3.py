...
def load_dataset():
    ...
    X, images, y = data['data'], data['images'], data['target']
...
def compute_mu_image(X):
    ...
    mu_image = np.mean(X, axis=0).reshape((64,64))
...
def compute_eigen_faces(X, k=20):
    ...
    Xp, P = p2.PCA(X, k)
    P_images = [vector_to_image(P.T[i]) for i in range(k)]
