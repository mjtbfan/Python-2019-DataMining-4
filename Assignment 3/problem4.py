...
def predict(R, S, i, j):
    '''
        ...
        Input:
            R: the rating matrix, a float numpy matrix of shape m by n. Here m is the number of movies, n is the number of users. <- FALSE
               R[i,j] represents the rating of the j-th user on the i-th movie, and the rating could be 1,2,3,4 or 5
    ...
    sum = 0
    iter = 0
    for index, obj in enumerate(R[i, :]):
        if (obj is not None) & (index != j):
            iter += 1
            sum += obj
