...
def compute_L(R,U,V):
    ...
    L = np.ndarray(B.shape)

    for indx_i, i in enumerate(B):
        for indx_j, j in enumerate(i):
            if j:
                #print(R[indx_i, indx_j] - np.dot(U[i, :], V[:, j]))
                L[indx_i, indx_j] = R[indx_i, indx_j] - np.dot(U[indx_i, :], V[:, indx_j])
            else:
                L[indx_i, indx_j] = 0
